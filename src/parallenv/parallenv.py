...
class ParallEnv(Generic[ObsType, ActType]):
    ...
    def __init__(
        ...
        self.consumer_worker = threading.Thread(
    ...
    def _split_by_workers(
        ...
        per_worker_ids = [
            self._filter_seq(env_ids, worker_mask)
            for worker_mask in per_worker_masks
        ]
        worker_indexes = [i for i, worker_env_ids in enumerate(per_worker_ids) if len(worker_env_ids)]
        per_worker_ids = [worker_env_ids for worker_env_ids in per_worker_ids if len(worker_env_ids)]
        per_worker_masks = [mask for i, mask in enumerate(per_worker_masks) if i in worker_indexes]
        ...
        for seq in seqs:
            per_worker_seqs.append(
                [
                    self._filter_seq(seq, worker_mask)
                    for worker_mask in per_worker_masks
                ]
            )
        ...
        *,
        ...
        if env_ids is None:
            ...
            if not self._check_envs_states_default(env_ids):
                raise AlreadyPendingEnvError("There are environments waiting for a pending call")
            for id_ in env_ids:
        ...
        worker_indexes, per_worker_ids, (seeds,) = self._split_by_workers(env_ids, [seed])
        for worker_idx, worker_env_ids, worker_seed in zip(worker_indexes, per_worker_ids, seeds):
    ...
    def step(self, env_ids: Sequence[int], actions: ActType) -> None:
        ...
        worker_indexes, per_worker_ids, (actions,) = self._split_by_workers(env_ids, [actions])
        for worker_idx, worker_env_ids, worker_actions in zip(worker_indexes, per_worker_ids, actions):
            self.command_queues[worker_idx].put(
                (
                    "step",
                    (
    ...
    def close(self) -> None:
        ...
        connection.wait(
            [p.sentinel for p in self.processes]
        )
    ...
    while True:
        ...
        if command == "step":
            ...
            for i, id_ in enumerate(ids):
                ...
                experience_queue.put(
                    (id_, (observation, None, None, None, info))
                )
        elif command == "close":
            break
    experience_queue.put(
